...
class PlayersSelection extends Component {
  ...
  isButtonDisabled = (player) => {
    return (
      this.state.selectedPlayers.includes(player.name) ||
      Number(player.points) + this.state.counter > 75
    );
  };
    ...
    return (
      <>
        ...
        <section className="section1">
          ...
          {players.map((player) => (
            <div key={player.id}>
              ...
              <button
                disabled={!this.state.selectedPlayers.includes(player.name)}
                onClick={() => this.removeHandler(player)}
              >
                Remove Player
              </button>
            </div>
        ...
        <section className="section2">
          <h1>
            Selected Players ({this.state.selectedPlayers.length}) - (
            {this.state.counter})
          </h1>
